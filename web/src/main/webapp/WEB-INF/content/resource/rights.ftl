...
<div class="row">
    <div class="span12">
    <p><i>This ${persistable.resourceType.label?lower_case} is currently shared with ${proxies?size} <#if (proxies?size == 1)>person<#else>people</#if><#if (persistable.managedResourceCollections?size > 0)>, and is in ${persistable.managedResourceCollections?size} collections</#if>.</i><br></p>
    <@s.hidden name="id" />
    <#if editor>
    <#-- 
