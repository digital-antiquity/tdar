...
<div class="row">
    <div class="span12">
    <p><i>This resource is shared with ${proxies?size} <#if (proxies?size == 1)>person<#else>people</#if><#if (persistable.sharedCollections?size > 0)>, and is in ${persistable.sharedCollections?size} collections</#if>.</i><br></p>
    <@s.hidden name="id" />
    <#if editor>
    <#-- 
