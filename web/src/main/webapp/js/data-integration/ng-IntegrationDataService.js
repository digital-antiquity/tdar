        ...
        integration.columns.forEach(function(column) {
            ...
            if (column.type == 'integration') {
                ...
                .map(function(selection) {
                    return selection.dataTableColumn;
                })
                ...
                // fixme: display columns have 'dataTableColumnSelections', which is virtually identical to 'selectedDataTableColumns'. refactor so they use the
                // same name
                outputColumn.dataTableColumns = (column.dataTableColumnSelections.filter(function(col) {
                    return !!col
                }).map(function(col) {
...
function DataService($http, $cacheFactory, $q, $log, $rootScope) {
    ...
    /**
     * Returns data from embedded dom element using script tags of type "application/json". If name specified, return the object contained in the script id of
     * the same name, otherwise return map<name, object> of all embedded objects, keyed by script element's ID.
     * 
     * @param name
     * @returns {*}
     */
    ...
    function _loadIntegrationById(id, integration) {
        ...
        httpPromise.success(function(data) {
            // console.log(data);
            _loadIntegration(data, integration).then(function() {
                futureData.resolve()
            }, function() {
                futureData.reject()
            });
    ...
    function _loadIntegration(json, integration) {
        ...
        self.loadTableDetails(dataTableIds).then(function(dataTables) {
            ...
            dataTables = dataTables.filter(function(dt) {
                return !!dt
            });
            ...
            var futureOntologyDetails = self.loadOntologyDetails(json.columns.filter(function(c) {
                return c.type === 'INTEGRATION';
            }).map(function(c) {
                return c.ontology.id;
            }));
    ...
    this.loadUpdatedParticipationInformation = function(integration) {
        ...
        var unprocessedIds = integration.getMappedDataTableColumns().filter(function(col) {
    ...
    this.loadTableDetails = function(dataTableIds) {
        ...
        if (missingTableIds.length > 0) {
            ...
            httpPromise.success(function(data) {
                ...
                data.mappedOntologies.filter(function(ontology) {
                    return !ontologyCache.get(ontology.id)
    ...
    /**
     * 
     * Return an array of previously-loaded dataTables in the same order of the specified dataTableIds. Note it is not necessary to actively consult the cache
     * before attempting to load them. DataService.loadTableDetails() will automagically pull from cached results when available.
     * 
     * @param dataTableIds
     */
    ...
    this.loadNodeParticipation = function(dataTableColumnIds) {
        ...
        httpPromise.success(function(nodeIdsByColumnId) {
            ...
            Object.keys(nodeIdsByColumnId).forEach(function(dataTableColumnId) {
                ...
                var nodes = nodeIdsByColumnId[dataTableColumnId].map(function(ontologyNodeId) {
                    return ontologyNodeCache.get(ontologyNodeId)
                });
    ...
    this.processIntegration = function(integration) {
        ...
        $http({
            ...
            data : $.param({
                integration : JSON.stringify(_dumpObject(integration))
            })
