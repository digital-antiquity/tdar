...
app.service("ValidationService", [
        ...
        function(DataService, $q) {
            ...
            this.validateJson = function(jsonData) {
                ...
                .filter(function(c) {
                    return TYPE_INTEGRATION === c.type
                })
                // exclude valid dataTableColumns
                .filter(function(ic) {
                    return ic.dataTableColumns.every(function(col) {
                        return col == null
                    })
                })
                // convert invalid columns to error messages
                .map(function(invalidColumn) {
                    return "The following integration column is invalid:" + invalidColumn.name;
                }));

                // rule: columns must have the correct number of dataTableColumns
                _addErrors(jsonData.columns.filter(function(c) {
                    return c.dataTableColumns && jsonData.dataTables;
                }).filter(function(c) {
                    return TYPE_COUNT !== c.type;
                }).filter(function(c) {
                    return c.dataTableColumns.length !== jsonData.dataTables.length;
                }).map(function(invalidColumn) {
                    return "Column '" + invalidColumn.name + "' has the wrong number of data table columns"
                }));
             ...
             * @param integration
             * @param jsonData
             *            the original, deflated integration
            ...
            this.validateIntegration = function(jsonData, integration) {
                ...
                var expectedTableIds = jsonData.dataTables.map(function(dt) {
                    return dt.id
                });
                var actualTableIds = integration.dataTables.map(function(dt) {
                    return dt.id
                });
                var missingTableIds = expectedTableIds.filter(function(id) {
                    return actualTableIds.indexOf(id) === -1
                });
                ...
                .filter(function(c) {
                    return !c.nodeSelection.every(function(node) {
                        var cachedNode = DataService.ontologyNodeCache.get(node.id);
                        if (!cachedNode) {
                            console.warn("node id not found:%s", node.id)
                        }
                        return cachedNode
                    })
                })
                ...
                .map(
                        function(invalidColumn) {
                            return "The following ontology has been modified since creating this integration:'" + invalidColumn.ontology.title +
                ...
                .reduce(function(dtclist, c) {
                    return dtclist.concat(c.dataTableColumns
