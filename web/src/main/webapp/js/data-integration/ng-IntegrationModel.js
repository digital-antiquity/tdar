...
function Integration(dataService) {
    ...
    /**
     * Name for the current integration workflow, specified by the user. This is for organizational purposes only(e.g. picking a previous integration out of a
     * list). This name does not appear in the final integration report.
     * 
     * @type {string}
     */
    self.title = "";
     ...
     * object's dataTables. That is, the system can only create an integration column for a given ontology if, for every dataTable in Integration#dataTables,
     * there is at least one datataneColumn with a default_ontology_id that equals ontology.id.
     * 
     * @type {Array}
     */
    self.ontologies = [];
    ...
    self.addIntegrationColumn = function _addIntegrationColumn(title, ontology) {
        ...
        /**
         * Initialize, or update, the selected dataTable column selections. Each columnSelection object contains a 'dataTable' and a 'dataTableColumn' property.
         * If a dataTable has no compatible dataTableColumns, selection.dataTableColumn is null.
         * 
         */
    ...
    /**
     * Remove specified dataTables from the viewmodel. This impacts all integration columns and any display columns that have dataTableColumns which belong to
     * any of the specified dataTables.
     * 
     * @param dataTables
     * @private
     */
        ...
        self.columns.forEach(function(column) {
            if (column.type != 'integration') {
                addedDataTables.forEach(function(table) {
                    column.dataTableColumnSelections.push({
                        dataTable : table,
                        dataTableCoumn : null
                    });
                });
    ...
    /**
     * Return the list of "shared" ontology id's. An ontology is considered to be shared if it is mapped to at least two data table columns in our list of data
     * tables.
     * 
     * @returns {*}
     */
    ...
    /**
     * Add a 'display column' to the columns list. A display column contains a list of dataTableColumn selections, which the system will include in the final
     * integration report file. The user may choose 0 or 1 dataTableColumn from each dataTable. This method primes the dataTableColumnSelections list to be size
     * N with each slot containing null (where N is the count of data tables in the current integration workflow.
     * 
     * @param title
     * @private
     */
    ...
    /**
     * Add specified dataTable to the viewModel.dataTables array. Duplicates are ignored. This is the recommended way of manipulating this list, as opposed to
    ...
    /**
     * Return 'tri-state' value indicating the participation status of the specified node w.r.t. the specified dataTableColumn. True means the node is present
     * in the column, False means it does not, and NULL means that IntegrationModel is currently loading participation data for the specified dataTableColumn.
     * 
