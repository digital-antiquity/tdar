 ...
 *            eventedd object
 * @param object
 *            tag tag to help differentiate event messages
 */
function _logEventedClass(object, tag) {
    var eventNames = [ 'baselayerchange', 'overlayadd', 'overlayremove', 'layeradd', 'layerremove', 'zoomlevelschange', 'resize', 'unload', 'viewreset',
            'load', 'zoomstart', 'movestart', 'zoom', 'move', 'zoomend', 'moveend', 'popupopen', 'popupclose', 'autopanstart', 'locationerror' ];
 ...
 * 
 * @param elem
 *            the init function binds the map to the specified element via data-attribute named 'map'
...
function _initMap(elem) {
    ...
    var _bodyData = {
        leafletApiKey : _bdata.leafletApiKey,
        leafletTileProvider : _bdata.leafletTileProvider
    };
    if (_bdata.centerlat && _bdata.centerlong) {
        _bodyData.center = {
            lat : _bdata.centerlat,
            lng : _bdata.centerlong
        }
    }
    ;
...
/**
 * Initialize any 'results style' maps and return a list of promises equal in size to the number of maps. Each promise resolves when leaflet completes loading.
 * 
 * @returns {Array|*}
 * @private
 */
 ...
 *            the leaflet map
 * @param markers
 *            clustergroup (jtd: I think) of existing map markers
 * @param data
 *            object containing list of leaflet feature objects (in data.features). These features represent one "page" of data
 * @param startRecord
 *            record number of the first record in the current page
 * @param bFitBounds
 *            if true, this method tells the map to pan/zoom to fit updated set of markers
 * @private
 */
function _update(map, markers, data, startRecord, bFitBounds, animate) {
    // translate datapoints to list of markers
    var layers = data.features.filter(function(feature) {
        return feature.geometry.hasOwnProperty("type");
    }).map(function(feature) {
        var title = feature.properties.title;
        var c = feature.geometry.coordinates;
        var marker = L.marker(new L.LatLng(c[1], c[0]), {
            title : $.trim(title)
        });
...
function _initLeafletMaps() {
    return $(".leaflet-map").map(function() {
        var $el = $(this);
        return _initMap(this).done(function(map) {
...
function _getMaps() {
    ...
    return $('.leaflet-container').map(function(i, elem) {
        return $(elem).data().map;
    });
}
