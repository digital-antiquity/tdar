 ...
 * 
 * Useful for wiring up an entire form (e.g. after we reconstruct completed form DOM after back-button navigation), or a single div (e.g. when user adds a new
 * search term).
 * 
 * @param containerElem
 * @private
...
/**
 * Perform a stringformat on the specified element attribute value, using a map to serve as the search/replace values.
 * 
 * @param $elem
 *            element
 * @param attr
 *            name of attribute value to format
 * @param map
 *            jsobject with replacement mappings.
 * @private
 */
...
/**
 * Modify certain attributes within container element so to indicate they belong to the specified group, row number (according to the attribute naming
 * conventions that we use for elements in a 'repeatrow' section, e.g "<input type=hidden id="personRow_1_5"> would be the id value for an element in group#2,
 * row 6).
 * 
 * @param $root
 *            container element
 * @param groupnum
 *            group number (at time of writing, only one search 'group' is supported, so this is always '0'
 * @param rownum
 *            row number
 * @private
 */
 ...
 * 
 * Useful when rendering the page when user navigates to the search form via a back-button navigation. We rely on ubiquitous (but non-standard) browser behavior
 * to recover this state and rebuild the DOM as it existed at the moment the user submitted the search form.
 * 
 * @private
 */
 ...
 * 
 * This is done during serialization so that the 'select, checkbox, and radio inputs will render with the last -selected value prior to when the user submitted
 * the form.
 * 
 * @param $formElements
