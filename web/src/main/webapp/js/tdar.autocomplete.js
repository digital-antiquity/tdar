...
/**
 * Autocomplete "object cache". Some autocomplete fields allow for the adhoc creation of new values. When the user creates new adhoc records they are stored
 * in the object cache. If the user performs subsequent autocomplete lookups, the results of the server lookup are combined with a search of the records of
 * the object cache and any matching records in the cache are included in the autocomplete results list. This makes data entry easier for situations where a
 * user intends make repeated references to an adhoc record in a form.
 * 
 * 
 * @param acOptions
 *            url: the url used by the ajax lookup - used to filter which adhoc records are returned by a search objectMapper: function used to map an a set
 *            of form fields to record object
 * 
 * @constructor
 */
...
/**
 * Determines if the fields representing an autocomplete control should be interpreted as "empty". For example, if all of the fields in a person
 * autocomplete fieldset have empty value attributes, this function would return true.
 * 
 * @param element
 ...
 *            results (default: add objectCach search results), requestData: {} additional name/val request data to include in ajax request, url: url for
 *            ajax request, showCreate: boolean. if true, allow user to create a new record by specifying value that cannot be found on the server.
 * 
 * 
 */
...
/**
 * return an object from any autocomplete input elements inside the specified parentElem elment. This function maps every .autocomplete-ui-input into a
 * property of the returned object. The property name is based on the value of the "autocompletename" attribute of the input element (or the value of the
 * 'name' attribute, if no autocompletename attribute specified.
 * 
 * @param parentElem
 */
