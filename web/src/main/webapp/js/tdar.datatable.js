...
core.namespace("datatable");
    ...
    function fnRenderTitle(oObj, b, c) {
        ...
        var html = common.htmlEncode(objResource.title);
        if (link == true) {
            html = '<a href="' + core.uri(objResource.urlNamespace + '/' + objResource.id) + '" class=\'title\'><b>' +
                    common.htmlEncode(objResource.title) + '</b></a>';
        }
    ...
    function fnRenderTitleAndDescription(a, b, oObj, d) {
        ...
        var objResource = oObj;
        return fnRenderTitle(oObj) + '<br /> <span>' + common.htmlEncode(core.ellipsify(objResource.description, 80)) + '</span>';
    }
    ...
    function _registerUserLookupDatatable() {
        ...
        return _registerLookupDataTable(settings);
    }
            ...
            if (viewRowSupported) {
                if (f.displayName == 'Row Id') {
                    col.render = function(data, type, row, meta) {
                        var url = core.uri(namespace + '/row/' + resourceId + '/' + dataTableId + '/' + row[1]);
