...
/**
 * Register a new dtatable control. By default, the datatable populates with resources editable by the user. Most of the initialization options required by the
 * $.dataTable are handled by this function. However, you can override these functions by via the options argument in the constructor
 * 
 * @param parms
 *            options object - properties in the object will override the defaults used to create the datatable. see the DataTables.net documentation for full
 *            list of option. Tdar-specific properties include: tableSelector: jquery selector which contains the table to initialize/convert into a datatable
 *            requestCallback: callback executed prior to performing ajax request. if it returns an object, those property name/value pairs are included in the
 *            ajax request, selectableRows: boolean indicating whether the rows of the data table are selectable, rowSelectionCallback: callback executed
 *            whenever user selects a row , "sAjaxSource": '/lookup/resource': url to request rowdata
 * 
...
function fnRenderIdColumn(oObj) {
    ...
    return ('<label class="datatable-cell-unstyled">' + '<input type="checkbox" class="datatable-checkbox ' + resourceType + '" id="' + attrId + '" value="' +
            id + '" >' + id + '</label>');
}
...
function fnRenderTitle(oObj) {
    ...
    var html = '<a href="' + core.uri(objResource.urlNamespace + '/' + objResource.id) + '" class=\'title\'>' + common.htmlEncode(objResource.title) + '</a>';
...
function _addRow($dataTable, $table, idattr, obj, action) {
    ...
    var $tr = $("<tr><td>" + obj.id + '<input type="hidden" name="' + action + '" value="' + obj.id + '" id="hrid' + obj.id + '">' + "</td><td>" + obj.title +
            "</td></tr>");
    $tr.attr("id", idattr);
    $table.append($tr);
}
