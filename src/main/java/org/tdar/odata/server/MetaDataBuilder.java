...
public class MetaDataBuilder implements IMetaDataBuilder {
    ...
    // A question requiring careful consideration is whether each TDataTable represents a different entity
    // as all of the fields are often different. I am leaning towards the notion of an abstract TDataTable
    ...
    public EdmDataServices build() {
        ...
        try {
            ...
            EdmDataServices.Builder metadata = EdmDataServices.newBuilder()
                    .setVersion(ODataConstants.DATA_SERVICE_VERSION)
                    .addSchemas(schemaBuilders);
    ...
    private void containerBuilders() {
        EdmEntityContainer.Builder entityContainerBuilder = EdmEntityContainer.newBuilder()
                .setName(Container.NAME)
                .setIsDefault(true)
                .addEntitySets(entitySetBuilders)
                .addAssociationSets(associationSets);
        entityContainerBuilders.add(entityContainerBuilder);
    }

    private void schemaBuilders() {
        Builder schema = EdmSchema.newBuilder()
                .setNamespace(namespace)
                .addEntityTypes(entityTypeBuilders)
    ...
    private void entitySetTypePropertyAssociationBuilders()
    {
        ...
        {
            abstractDateRecordEntityTypeBuilder = EdmEntityType.newBuilder()
                    .setNamespace(namespace)
                    .setName(Entity.T_DATA_RECORD)
                    .addKeys(Property.ID)
                    .addProperties(propertyBuildersFromRecord());

            abstractDataRecordEntitySetBuilder = EdmEntitySet.newBuilder()
                    .setName(EntitySet.T_DATA_RECORDS)
        ...
        EdmEntitySet.Builder dataRecordEntitySetBuilder = null;
        {
            ...
            for (DataTable dataTable : ownedDataTables)
            {
                ...
                dateRecordEntityTypeBuilder = EdmEntityType.newBuilder()
                        ...
                        .setBaseType(Entity.T_DATA_RECORD)
                        .addKeys(Property.ID)
                        .addProperties(propertyBuildersFromRecord(dataTableColumns));

                dataRecordEntitySetBuilder = EdmEntitySet.newBuilder()
                        .setName(dataTable.getName() + "_s")
        ...
        {
            dataTableEntityTypeBuilder = EdmEntityType.newBuilder()
                    .setNamespace(namespace)
                    .setName(Entity.T_DATA_TABLE)
                    .addKeys(Property.NAME)
                    .addProperties(
                            EdmProperty.newBuilder(Property.ID).setType(EdmSimpleType.INT64),
                            EdmProperty.newBuilder(Property.NAME).setType(EdmSimpleType.STRING));

            dataTableEntitySetBuilder = EdmEntitySet.newBuilder()
                    .setName(Property.T_DATA_TABLES)
        ...
        {
            dataSetEntityTypeBuilder = EdmEntityType.newBuilder()
                    .setNamespace(namespace)
                    .setName(Entity.T_DATA_SET)
                    .addKeys(Property.NAME)
                    .addProperties(
                            EdmProperty.newBuilder(Property.ID).setType(EdmSimpleType.INT64),
                            EdmProperty.newBuilder(Property.NAME).setType(EdmSimpleType.STRING));

            dataSetEntitySetBuilder = EdmEntitySet.newBuilder()
                    .setName(EntitySet.T_DATA_SETS)
    ...
    private EdmAssociation.Builder defineAssociation(
            ...
            EdmMultiplicity fromMultiplicity,
            EdmMultiplicity toMultiplicity,
        ...
        // add EdmAssociation
        EdmAssociationEnd.Builder fromAssociationEnd = EdmAssociationEnd.newBuilder()
                .setRole(fromEntityType
                        .getName())
                .setType(fromEntityType)
                .setMultiplicity(fromMultiplicity);
        ...
        EdmAssociationEnd.Builder toAssociationEnd = EdmAssociationEnd.newBuilder()
                .setRole(toAssociationRole)
                .setType(toEntityType)
                .setMultiplicity(toMultiplicity);
