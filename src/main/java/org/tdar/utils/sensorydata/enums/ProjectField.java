...
public enum ProjectField {
    ...
    private static Map<String, ProjectField> labelsToEnums;
    static {
        labelsToEnums = new HashMap<String, ProjectField>();
        for(ProjectField field : ProjectField.values()) {
            labelsToEnums.put(field.label, field);
        }
    }    
    
    private ProjectField(String label) {
        this.label = label;
    }
    
    public static ProjectField fromLabel(String label) {
