package org.tdar.core.service.external.auth.authenticationProviders;
...
public class CrowdSoapService extends BaseAuthenticationProvider {
    ...
    public boolean addUser(Person person, String password) {
        ...
        try {
            ...
            securityServerClient.addPrincipalToGroup(email, TdarGroup.CONFLUENCE_USERS.getGroupName());
        } catch (RemoteException e) {
            e.printStackTrace();
    ...
    public void resetUserPassword(Person person) {
        ...
        try {
            securityServerClient.resetPrincipalCredential(person.getEmail().toLowerCase());
        } catch (Exception e) {
    ...
    public void updateUserPassword(Person person, String password) {
        ...
        // TODO all manner of validation required here
        try {
            securityServerClient.updatePrincipalCredential(person.getEmail().toLowerCase(), credential);
        } catch (Exception e) {
    ...
    @Override
    public String[] findGroupMemberships(String email) {
        try {
            return securityServerClient.findGroupMemberships(email);
        } catch (RemoteException e) {
            logger.error("Caught RemoteException while trying to contact the crowd server", e);
            throw new RuntimeException(e);
        } catch (InvalidAuthorizationTokenException e) {
