...
//    public static final String QUERY_MATCHING_FILES = "datasetRelated.Files";
    ...
    public static final String UPDATE_KEYWORD_OCCURRENCE_COUNT_INHERITANCE = "update %1$s set occurrence = occurrence + (select count(resource_id) from resource_%1$s where %1$s_id =%1$s.id and resource_id in (select project_id from information_resource where %2$s is true) group by %1$s_id)";
    public static final String UPDATE_KEYWORD_OCCURRENCE_COUNT= "update %1$s set occurrence =  occurrence + (select count(resource_id) from resource_%1$s where %1$s_id =%1$s.id group by %1$s_id)";
    public static final String UPDATE_CREATOR_OCCURRENCE_CLEAR_COUNT = "update creator set occurrence=0";
    public static final String UPDATE_CREATOR_OCCURRENCE_RESOURCE_INFORMATION_RESOURCE_PUBLISHER = "update creator set occurrence=occurrence + (select count(information_resource.id) from information_resource where publisher_id=creator.id group by publisher_id)";
    public static final String UPDATE_CREATOR_OCCURRENCE_RESOURCE_INFORMATION_RESOURCE_PROVIDER = "update creator set occurrence=occurrence + (select count(information_resource.id) from information_resource where provider_institution_id=creator.id group by provider_institution_id)";
