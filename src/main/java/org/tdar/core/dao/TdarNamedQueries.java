...
public interface TdarNamedQueries {
    ...
    String UPDATE_BROWSE_CREATOR_OCCURRENCE_RESOURCE = "update creator set browse_occurrence = browse_occurrence + coalesce((select count(resource_id) from resource_creator where creator_id=creator.id and role in (%s) group by creator_id),0) ";
    String UPDATE_CREATOR_OCCURRENCE_RESOURCE_INHERITED = "update creator cc set browse_occurrence = (select (select count(r.id) from resource r where r.status='ACTIVE' and (submitter_id =creator.id or r.id in (select id from information_resource ir where publisher_id=creator.id  or provider_institution_id=creator.id)) and r.id not in (select resource_id from resource_creator where role in (%1$s)) and r.id not in (select id from information_resource ir where ir.id=r.id and ir.inheriting_individual_institutional_credit is true and ir.project_id  in (select resource_id from resource_creator where role in (%1$s)))) from creator where creator.id=cc.id group by creator.id)";
    String UPDATE_BROWSE_CREATOR_OCCURRENCE_RESOURCE_INHERITED = "update creator set browse_occurrence = browse_occurrence+ coalesce((select count(resource_id) from resource_creator where creator_id=creator.id and resource_id in (select project_id from information_resource where inheriting_individual_institutional_credit is true) and role in (%s)  group by creator_id),0) ";
    ...
    String UPDATE_BROWSE_CREATOR_OCCURRENCE = "update creator set browse_occurrence = browse_occurrence + coalesce((select count(resource_id) from resource_creator where creator_id=creator.id and resource_id in (select project_id from information_resource where inheriting_individual_institutional_credit is true) and role in (%s)  group by creator_id),0) ";
