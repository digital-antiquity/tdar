...
public interface TdarNamedQueries {
    ...
    String ANNUAL_ACCESS_SKELETON = "select id, title, resource_type, status, %s (select sum(count) from resource_access_day_agg where resource_id=resource.id) as \"Total Views\", %s (select sum(count) from file_download_day_agg, information_resource_file where information_resource_id=resource.id and information_resource_file.id=file_download_day_agg.information_resource_file_id) as \"Total Downloads\" from resource where id in (:ids)";
