...
public interface TdarNamedQueries {
    ...
    String BROWSE_CREATOR_ROLES_2 = "insert into rctest select resource_id, creator_id, 1 from resource_creator where resource_id in (select rid from rctest) and role in (%1$s)";
    String BROWSE_CREATOR_IR_ROLES_3 = "insert into rctest select resource_id, creator_id, 1 from resource_creator where resource_id in (select project_id from rctest, information_resource ir where rid=ir.id and inheriting_individual_institutional_credit=true ) and role in (%1$s)";
