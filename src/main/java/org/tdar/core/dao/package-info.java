...
//                query = "select sum(size) from InformationResource res left join res.informationResourceFiles irf left join irf.informationResourceFileVersions as irfv where submitter.id in (:submitterIds) and status in (:statuses) and irfv.fileVersionType in (:types)"
//        ),
//        @org.hibernate.annotations.NamedQuery(
