...
public class DownloadController extends AuthenticationAware.Base implements DownloadHandler {
    ...
    public String execute() throws TdarActionException {
        ...
        if (!getAuthenticationAndAuthorizationService().canDownload(irFileVersion, getAuthenticatedUser())) {
            String msg = String.format("user %s does not have permissions to download %s", getAuthenticatedUser(), irFileVersion);
            getLogger().warn(msg);
            return FORBIDDEN;
        }
        setInformationResourceId(irFileVersion.getInformationResourceId());
        getLogger().info("user {} downloaded {}", getAuthenticatedUser(), irFileVersion);
    ...
    public String downloadZipArchive() throws TdarActionException {
        ...
        for (InformationResourceFile irf : ir.getInformationResourceFiles()) {
            ...
            if (!getAuthenticationAndAuthorizationService().canDownload(irf, getAuthenticatedUser())) {
                getLogger().warn("thumbail request: resource is confidential/embargoed:" + informationResourceFileId);
                return FORBIDDEN;
            }
