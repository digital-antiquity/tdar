...
<br/>
	...
	<div id='divCodingSheet-${column_index}' class="codingInfo" tooltipcontent="#codingSheetToolTip" tiplabel="Coding Sheet">
        ...
        <b>Translate your data using a Coding Sheet:</b>
			...
			<small><a target="_blank" href='<@s.url value="/coding-sheet/add?returnToResourceMappingId=${resource.id?c}"/>'>Create Coding Sheet</a> </small>
			<br/>
	</div>
	 <div id='divOntology-${column_index}' class="ontologyInfo" tooltipcontent="#ontologyToolTip" tiplabel="Ontology">
        <b>Map it to an Ontology:</b><br/>
			...
			<small><a target="_blank" href='<@s.url value="/ontology/add?returnToResourceMappingId=${resource.id?c}"/>'>Create Ontology</a> </small>
	</div>
    <br/>
	<div class="mappingInfo" tooltipcontent="#mappingToolTip" tiplabel="Mapping tDAR Resources">
	...
	<fieldgroup class="mappingDetail">
	   <legend>tDAR can associate groups of documents and images with this dataset as long as they're all part of the same project. 
	   If this column has filenames in it, tDAR will associate the filename with the filename of the image or document and load the row
	   data as additional fields in the tDAR record.
	   </legend>
...
<#--
	...
	<#else>
	<i>cannot map this dataset to a set of tDAR resources because the dataset is not in a project</i>
	</#if>
	</div>
</div>
</#list>
...
<div class="hidden" style="visibility:hidden;display:none">
	<span class="hidden" id="generalToolTip">
		 Each "column" subform shown on the table metadata page represents a column in the dataset, and provides fields to describe the data in that column. This is important documentation for researchers that wish to use the dataset, and where relevant the form links to coding sheets and tDAR ontologies to faciliate research. 
    </span>
	...
	<span class="hidden" id="codingSheetToolTip">
		If the data in this column is coded and the right coding sheet has been added to tDAR, please select a coding sheet that translates and explains the codes. 
    </span>
	...
	<span class="hidden" id="mappingToolTip">
		Use column values to map table rows to resources? - Select this check box if the data in the column links to a specific tDAR resource, usually an image or document.
    </span>
</div>
