...
$.validator
		.addMethod("currentyearorlater",
				function(value, element) {
					var intVal = parseInt(value);
					// allow -1 for internal management of things that don't
					// have
					// dates
					return (intVal == value && (intVal >= 1900 + (new Date())
							.getYear() && intVal < 2050));
				}, "a date in the last millenia is expected");
		...
		.addMethod(
				...
				function(phone_number, element) {
					...
					return this.optional(element)
							...
							&& phone_number
									.match(/^(1-?)?(\([2-9]\d{2}\)|[2-9]\d{2})(\s?-?)+[2-9]\d{2}(-?\s?)+\d{4}$/);
...
// FIXME: Modify .validIdRequired to support subtle+complex requirements for
// person, user, copyrightholder contexts
$.validator
		.addMethod(
				"validIdRequired",
				function(value, element) {
...
$.validator.addMethod("columnEncoding", function(value, element) {
	// when using this method on radio buttons, validate only calls this
	// function once per radio group (using the first element of the group for
	// value, element)
	// However, we still need to put put the error message in the title attr of
	// the first element in group. Feels hokey but there you are.
	...
	// if we came here by way of a form 're-validate', we need to make sure that
	// validation logic in registerCheckboxInfo happens first.
	registerCheckboxInfo.apply(element);

	if ($selectedElement.is(':disabled')) {
		var val = $selectedElement.val().toLowerCase().replace('_', ' ');
		element.title = "The selection '" + val
				+ "' is no longer valid for column " + displayName;
		return false;
	}
	...
	if (square == undefined || square.length == 0) {
		...
		element.title = "Column " + displayName
				+ " contains errors or invalid selections";
		return false;
	}
});
