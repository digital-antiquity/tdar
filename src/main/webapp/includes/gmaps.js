...
function TdarGMap(mapElement, allowRegionSelection) {
	...
	if (allowRegionSelection) {
		gzControl = new GZoomControl({
			...
			sButtonHTML : "<img src='" + getBaseURI()
					+ "images/select-region.png' />",
			sButtonZoomingHTML : "<img src='" + getBaseURI()
					+ "images/select-region.png' />",
		...
		this.map.addControl(gzControl, new GControlPosition(
				G_ANCHOR_BOTTOM_LEFT, new GSize(10, 120)));
	}
	this.centerToUs();
