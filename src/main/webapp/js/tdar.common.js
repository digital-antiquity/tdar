...
//         gets added to the dom.
    ...
    function logarithm(base, n) {
      return Math.log(n) / Math.log(base);
    }

    function friendlySize(bytes, si) {
        var thresh = si ? 1000 : 1024;
        var units = si ? ['kB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'] : ['KiB', 'MiB', 'GiB', 'TiB', 'PiB', 'EiB', 'ZiB', 'YiB'];
        var logbytes = logarithm(thresh, bytes);
        var idx = Math.floor(logbytes);
        var base = Math.pow(thresh, idx );
        var baseUnits = bytes / base;
        return {amt: baseUnits, unit: units[idx], msg: baseUnits.toFixed(0) + " " + units[idx]};
    }
