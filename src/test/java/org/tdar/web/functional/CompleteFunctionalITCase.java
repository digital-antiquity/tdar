...
//                docValMap.put(TestConstants.COPYRIGHT_HOLDER_TYPE, "Institution");
        ...
        public void testCreateDocumentEditSavehasResource() {
            ...
            assertTrue(getSource().contains(ORIGINAL_START_DATE));
            assertTrue(getSource().contains(ORIGINAL_END_DATE));
            ...
            logger.info(find("body").getText());
            assertTrue("expecting to be on view page. Actual path:" + path + "\n" + find("body").getText(), path.matches(REGEX_DOCUMENT_VIEW));
            assertTrue(getSource().contains(NEW_START_DATE));
            assertFalse(getSource().contains(ORIGINAL_START_DATE));
            assertTrue(getSource().contains(ORIGINAL_END_DATE));
        ...
        public void testCreateDocument() {
            ...
            //things we expect to find on the view page that are visible to the user
            List<String> expectedInText = new ArrayList<>();
            //things we expect to find in the source code of the view page
            List<String> expectedInSource = new ArrayList<>();

            
            //FIXME: improve this section by a) having this forloop build a list of expected strings  b) on failure, show all missing strings (not just first) 
            for (String key : docValMap.keySet()) {
                // avoid the issue of the fuzzy distances or truncation... use just
                // the top of the lat/long
                String value = docValMap.get(key);
                
                if (key.startsWith("latitudeLongitudeBox")) {
                    expectedInText.add(value.substring(0, value.indexOf(".")));
                    // these are displayed by "type" or not "displayed"
                } else if (valueBelongsInViewPage(key)) {
                    expectedInText.add(value);
                } 
            }
            for (String alt : alternateTextLookup) {
                expectedInText.add(alt);
            }
            for (String alt : alternateCodeLookup) {
                expectedInSource.add(alt);
            }
            expectedInText.add("embargo");
            
            for (String key : docMultiValMapLab.keySet()) {
                for (String val : docMultiValMapLab.get(key)) {
                    expectedInText.add(val);
                }
            }
            
            //TODO: loop through expected values,  determine the missing values, and assert that missing values list is empty.
            ...
            for (String key : docValMap.keySet()) {
                ...
                if (docUnorderdValMap.containsKey(key)) {
                    assertTrue(getText().contains(docValMap.get(key)));
                } else {
            ...
            assertTrue(getSource().contains(TEST_DOCUMENT_NAME));

        }
        
        //does this field key correspond to a value that we expect to appear on the view page?
        private boolean valueBelongsInViewPage(String key) {
            return !key.equals("document.journalName") && !key.equals("document.bookTitle") && !key.startsWith("authorInstitutions")
