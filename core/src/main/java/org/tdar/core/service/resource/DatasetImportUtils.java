...
public class DatasetImportUtils {
    ...
    public static void copyValues(ImportColumn incomingColumn, DataTableColumn col) {
        ...
        for (String val : incomingColumn.getValues()) {
            ...
            if (!StringUtils.contains(val, TdarConfiguration.getInstance().getDatasetCellDelimiter())) {
                col.getValues().add(StringUtils.substring(val, 0,254));
                continue;
            }
            for (String str : StringUtils.split(val,TdarConfiguration.getInstance().getDatasetCellDelimiter())) {
                String trim = StringUtils.trim(StringUtils.substring(str, 0,254));
                if (StringUtils.isBlank(trim)) {
                    continue;
                }
                col.getValues().add(trim);
            }
        }
        col.getIntValues().addAll(incomingColumn.getIntValues());
        col.getFloatValues().addAll(incomingColumn.getFloatValues());
    }
